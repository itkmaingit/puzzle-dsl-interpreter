...
def handle_value_node(node: dict, indent_level: int = 1) -> tuple[list[str], str]:
    ...
    match vname:
        ...
        case ValueType.CYCLE:
            ...
            lines.append(f"{indent_str}{var_name} = cycle({variable}, board)")

        case ValueType.CROSS:
            # 例: cross(variable="dd") => int
            variable = args.get("variable")
            lines.append(f"{indent_str}{var_name} = cross({variable}, board)")
...
def generate_code(json_data: dict) -> str:
    ...
    lines.append(f"{INDENT}except PanicError as e:")
    lines.append(f"{INDENT}{INDENT}raise PanicError(e)")
    lines.append(f"{INDENT}except Exception:")
    lines.append(f"{INDENT}{INDENT}raise UnknownError")
    ...
    lines.append(f"{INDENT}{INDENT}{INDENT}{INDENT}{INDENT}raise FailedError('成功回数が少なすぎました。')")
    lines.append(f"{INDENT}{INDENT}{INDENT}{INDENT}if CORRECT_RANGE[1] < success:")
    lines.append(f"{INDENT}{INDENT}{INDENT}{INDENT}{INDENT}print(f'試行回数: {{samples}}, 成功回数: {{success}}')")
    lines.append(f"{INDENT}{INDENT}{INDENT}{INDENT}{INDENT}raise FailedError('成功回数が多すぎました。')")
    lines.append(f"{INDENT}{INDENT}except FailedError:")
    ...
    lines.append(f"{INDENT}{INDENT}{INDENT}{INDENT}process.kill()")
    ...
    lines.append(f"{INDENT}{INDENT}except PanicError as e:")
    lines.append(f"{INDENT}{INDENT}{INDENT}print(f'Panic')")
    lines.append(f"{INDENT}{INDENT}{INDENT}print(f'{{traceback.format_exc()}}')")
    lines.append(f"{INDENT}{INDENT}{INDENT}for future in futures:")
    lines.append(f"{INDENT}{INDENT}{INDENT}{INDENT}if not future.running():")
    lines.append(f"{INDENT}{INDENT}{INDENT}{INDENT}{INDENT}future.cancel()")
    lines.append(f"{INDENT}{INDENT}{INDENT}for process in executor._processes.values():")
    ...
    lines.append(f"{INDENT}{INDENT}{INDENT}executor.shutdown(wait=False, cancel_futures=True)")
    lines.append(f"{INDENT}{INDENT}{INDENT}sys.exit(1)")
    lines.append(f"{INDENT}{INDENT}except UnknownError:")
    lines.append(f"{INDENT}{INDENT}{INDENT}print('Unknown')")
    lines.append(f"{INDENT}{INDENT}{INDENT}for future in futures:")
    lines.append(f"{INDENT}{INDENT}{INDENT}{INDENT}if not future.running():")
    lines.append(f"{INDENT}{INDENT}{INDENT}{INDENT}{INDENT}future.cancel()")
    lines.append(f"{INDENT}{INDENT}{INDENT}for process in executor._processes.values():")
